...
class SolutionEquation extends ai6.Solution {
  constructor(v) { super(v) }

  static zero() {
    // return new Solution(Matrix.create(2,1,0));
    return new SolutionEquation(Matrix.create(3,1,0));
  }
  ...
  neighbor() {    // 多變數解答的鄰居函數。
    ...
    return new SolutionEquation(nx);                    // 傳回新建的鄰居解答。
  }

  energy() {      // 能量函數:計算 ||AX-B||，也就是 ||Y-B||
    var X = this.v;
    var Y = A.mul(X);
    return Y.sub(B).norm();
  }
...
module.exports = SolutionEquation                    // 將解答類別匯出。