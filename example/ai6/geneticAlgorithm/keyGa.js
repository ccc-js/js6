...
var uu6 = require('../../../js6/uu6')

class KeyGA extends ai6.GeneticAlgorithm {
  randomChromosome() { // 隨機產生一個染色體 (一個 16 位元的 01 字串)
    var bits=[];
    for (var i=0; i<KeyGA.key.length; i++) {
      var bit = uu6.randomInt(0,2);
      bits.push(bit);
    }
    return bits.join('');
  }
  ...
  crossover(p1,p2) {
    var cutIdx = uu6.randomInt(0, p1.chromosome.length);
  ...
  mutate(chromosome) { // 突變運算
    var i=uu6.randomInt(0, chromosome.length); // 選擇突變點
    var cMutate=chromosome.substr(0, i)+
            uu6.randomChoose(['0','1'])+ // 在突變點上隨機選取 0 或 1
